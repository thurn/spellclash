...
await TAURI_INVOKE("connect");
},
async handleAction(clientData: ClientData, action: unknown) : Promise<void> {
await TAURI_INVOKE("handle_action", { clientData, action });
},
async updateField(clientData: ClientData, key: FieldKey, value: FieldValue) : Promise<void> {
await TAURI_INVOKE("update_field", { clientData, key, value });
},
async dragCard(clientData: ClientData, cardId: ClientCardId, location: CardOrderLocation, index: number) : Promise<void> {
await TAURI_INVOKE("drag_card", { clientData, cardId, location, index });
}
}
...
export type GameControlView = { button: GameButtonView } | { textInput: TextInputView } | { text: string }
/**
...
export type RevealedCardStatus = "selected" | "canSelect" | "canPlay" | { attacking: string } | { blocking: string }
/**
